...
async def read_summary(db: AsyncSession = Depends(get_db)):
...
async def update_expense(expense_id: UUID, expense: schemas.ExpenseUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_expense(expense_id: UUID, db: AsyncSession = Depends(get_db)):
    success = await crud.delete_expense(db, expense_id=expense_id)
    if not success:
        raise HTTPException(status_code=404, detail="Expense not found")
